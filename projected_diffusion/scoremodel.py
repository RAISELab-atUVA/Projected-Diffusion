...
class AnnealedLangevinDynamic():
    def __init__(self, sigma_min, sigma_max, n_steps, annealed_step, score_fn, projection_step, device, eps = 1e-1):
        '''
        ...
        self.pgd = projection_step is not None
    ...
    def _one_diffusion_step(self, x):
        ...
        for idx in range(len(self.process)):
            print(f"| model | Outer iter step {idx}.")
            if self.pgd: x = self._pgd_diffusion_step(x, idx)
            else: x = self._one_annealed_step(x, idx)
            yield x
            
    
    # One pgd step
    def _pgd_diffusion_step(self, x, idx, restore=False):
        '''
        x   : perturbated data
        idx : step of perturbation schedule
        '''            
        
        for step_num in tqdm(range(self.annealed_step)):
