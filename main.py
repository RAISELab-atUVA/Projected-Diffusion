...
def _train(args):
    ...
    # Set up trainer
    current_iteration = 0
    os.makedirs(os.path.join(f"examples/{args.projection_path}/models", args.run_name), exist_ok=True)
...
def _sample(args):
    ...
    projector = Projection() # TODO: Fix to pass args into Projection class
    ...
    if args.model_path is not None:
        model.load_state_dict(torch.load(args.model_path, map_location=device))
    model.eval()
    dynamic = AnnealedLangevinDynamic(args.sigma_min,args.sigma_max, args.n_steps, args.annealed_step, model, projecter, device, eps= args.eps)
